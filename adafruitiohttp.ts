...
namespace grove {
    ...
    /**
     * Send HTTP GET request to Adafruit IO and return response
     */
    //% block="Adafruit IO GET|Username %username|Path %path|AIO Key %aioKey"
    //% group="UartWiFi"
    export function adafruitIOGet(username: string, path: string, aioKey: string): string {
        ...
        while (isWifiConnected && retry > 0) {
            ...
            let data = "GET /api/v2/" + username + "/" + path + " HTTP/1.1\r\n"
            ...
            // Wait for "SEND OK"
            waitAtResponse("SEND OK", "SEND FAIL", "ERROR", 5000)

            // Now read the actual HTTP response
            return readHttpResponse(3000)
        }
    ...
    function readHttpResponse(timeout: number = 2000): string {
        let buffer = ""
        let start = input.runningTime()
